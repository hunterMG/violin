...
from .targets import extract_target_candidates, normalise_target
...
def validate_source(source: object) -> tuple[dict[str, str] | None, str | None]:
    ...
    declared = normalise_target(metadata["target"])
    foreign: set[str] = set()
    for node in ast.walk(tree):
        if not isinstance(node, ast.Constant) or not isinstance(node.value, str):
            continue
        for candidate in extract_target_candidates(f"probe {node.value}"):
            normalized = normalise_target(candidate)
...
def prepare_execution(source: object) -> tuple[dict[str, str], Path]:
    ...
    if classification == "target_touching":
        try:
            from .execution import _commit_guard_state
            from .ptt import find_active_task, parse_ptt
