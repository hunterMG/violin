...
_REPAIR_TEMPLATES = {
    ...
    Path("state/coverage-matrix.yaml"): (
        "skills/pentest/templates/coverage-matrix.yaml",
        None,
    ),
    Path("state/methodology-gates.yaml"): (
        "skills/pentest/templates/methodology-gates.yaml",
        None,
    ),
}
...
def check_bootstrap(
    ...
    for path_str in _ARTIFACT_DIRECTORIES:
        directory = eng_dir / path_str
        if not directory.exists():
            result.add_error(f"missing artifact directory: {path_str}")
        elif not directory.is_dir():
            result.add_error(f"artifact path is not a directory: {path_str}")

    if eng_dir.exists() and not (eng_dir / "scope" / "scope.yaml").exists():
        result.add_info(
            "create scope with: cp skills/pentest/templates/scope-template.yaml <ENG_DIR>/scope/scope.yaml"
        )
    ...
    if eng_dir.exists() and not (eng_dir / "state" / "history.md").exists():
        result.add_info(
            'initialize command history with: echo "# Command History — $(date +%F)" > <ENG_DIR>/state/history.md'
        )
...
def _auto_repair_corrupt_artifacts(eng_dir: Path, result: BootstrapResult) -> BootstrapResult:
    ...
    for error in result.errors:
        e_norm = error.replace("\\", "/")
        if any(key in e_norm for key in repaired_keys):
            new_infos.append(f"resolved: {error}")
            continue
