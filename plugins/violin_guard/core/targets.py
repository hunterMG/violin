...
def resolve_target(
    ...
    if role:
        ...
        if role not in roles:
            # The agent may pass --role web out of habit when the scope has no
            # roles section. If exactly one unique target exists, resolve it
            # directly instead of erroring (the target is unambiguous).
            raw_values = [
                value.strip()
            ...
            unique_hosts = list(dict.fromkeys(normalize_target(v) for v in raw_values))
            if len(unique_hosts) == 1:
                return raw_values[0]
            raise ValueError(
                f"target role {role!r} is not defined in scope.yaml (roles: "
                f"{', '.join(roles) or 'none'}); scope has {len(unique_hosts)} distinct "
                f"targets, so omit role or pass host=<target>"
            )
        ...
        for value in raw_values:
            by_host.setdefault(normalize_target(value), value)
        if len(by_host) > 1:
...
def _research_hosts(scope: dict[str, Any]) -> set[str]:
    ...
    return {normalize_target(val) for val in _values(scope.get("research_hosts", []))}
...
def is_research_host(scope: dict[str, Any], candidate: str | None) -> bool:
    ...
    normalized = normalize_target(candidate) if candidate else ""
...
def _callback_hosts(scope: dict[str, Any]) -> set[str]:
    ...
    if not isinstance(assessment_hosts, dict):
        return set()
    return {normalize_target(val) for val in _values(assessment_hosts.get("callback_hosts", []))}
...
def check_scope_targets(
    ...
    *,
    ...
    policy = _TargetPolicy(
        ...
        excluded_urls={val for val in _values(exclusions.get("urls", []))},
        excluded_paths={val for val in _values(exclusions.get("paths", []))},
    )

    explicit = normalize_target(primary_target) if primary_target else ""
...
def _read_scope_cached(resolved_path: str, mtime_ns: int, size: int) -> dict[str, Any] | None:
    data = yaml.safe_load(Path(resolved_path).read_text(encoding="utf-8")) or {}
    return data if isinstance(data, dict) else None


def _clear_scope_cache() -> None:
    _read_scope_cached.cache_clear()


def _read_scope(path: Path) -> dict[str, Any] | None:
    """Parse ``scope.yaml`` once per mtime snapshot (any edit invalidates)."""
    if not path.exists():
        return None
    try:
        st = path.stat()
        return _read_scope_cached(str(path.resolve()), st.st_mtime_ns, st.st_size)
    except Exception:
        return None
...
def resolve_command_targets(
    ...
    """Extract and normalize candidate targets from command, primary target, or scope fallback."""
    targets = {normalize_target(candidate) for candidate in extract_target_candidates(command)}
    if primary_target:
        targets.add(normalize_target(primary_target))

    if not targets and isinstance(scope_data, dict):
        targets_sec = scope_data.get("targets", {})
        if isinstance(targets_sec, dict):
            for target_ip in targets_sec.get("ip_addresses", []) or []:
                if isinstance(target_ip, str) and target_ip.strip():
                    targets.add(normalize_target(target_ip))
