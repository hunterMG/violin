...
- **Delegate** — Use `violin_exec` with `background=true` for tracked long-running work. Do not start target-facing background jobs through raw `terminal`.
...
- **Hermes plugin:** `plugins/violin_guard/` registers typed guard tools (`violin_status`, `violin_record_ptt`, `violin_record_hypothesis`, `violin_exec`, `violin_review_batch`) that call the shared service directly. The plugin is required for target execution.
