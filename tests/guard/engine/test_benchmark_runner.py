...
def test_init_benchmark_engagement_seeds_flat_coverage_matrix(tmp_path: Path) -> None:
    """Bootstrap must emit exactly the flat keys required by the close gate."""
    eng_dir = tmp_path / "eng"
    init_benchmark_engagement(eng_dir, "http://test-target.local:8080")
    scope = yaml.safe_load((eng_dir / "scope" / "scope.yaml").read_text(encoding="utf-8"))
    matrix = yaml.safe_load(
        (eng_dir / "state" / "coverage-matrix.yaml").read_text(encoding="utf-8")
    )
    obligations = [str(item).lower() for item in scope["engagement"]["coverage_obligations"]]
    assert list(matrix["coverage"]) == obligations
    assert all(cell["status"] == "pending" for cell in matrix["coverage"].values())
    assert not any(
        isinstance(value, dict) and "routes" in value for value in matrix["coverage"].values()
    )
